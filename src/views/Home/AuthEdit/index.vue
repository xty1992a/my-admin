        ...
        <el-table :data="userList">
          ...
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              ...
              <el-button
                type="danger"
                @click="delUser(scope.row.token)"
                :disabled="scope.row.token === 'admin'"
                >删除
              </el-button>
            </template>
          </el-table-column>
          ...
          <el-table-column label="操作" width="180">
            <template slot-scope="scope">
              <el-button
                ...
                :disabled="scope.row.key === 'admin'"
                >编辑</el-button
              >
              <el-button
                type="danger"
                @click="delRole(scope.row.key)"
                :disabled="scope.row.key === 'admin'"
                >删除</el-button
              >
            </template>
...
export default {
  ...
  computed: {
    ...mapState("Router", ["rawRoutes"]),
    ...mapState("User", ["roleList", "userList"]),
    routes() {
      return fmtRoutes(
        filterRoutes(JSON.parse(JSON.stringify(this.rawRoutes)))
      );
    }
  }
};
